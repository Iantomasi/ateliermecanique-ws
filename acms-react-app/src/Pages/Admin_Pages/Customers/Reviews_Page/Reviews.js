...
function Reviews(){
    ...
    const handleCustomerClick = () => {
        navigate(`/reviews/${ customerId}/newReview`,customerId );
    };
    ...
    function getALlCustomerReviews() {
        customerService.getCustomerReviews(customerId)
            .then(res => {
                if (res.status === 200) {
                    setReviews(res.data);
                    setPublicContent(true);
                                ...
                                <table className="w-full table-auto">
                                    ...
                                    {reviews.map((review) => (
                                        <ReviewBlock
                                            key={review.reviewId}
                                            review={review}
                                            refreshReviews={getALlCustomerReviews()}
                                            navigate={navigate}
                                        />
                                    ))}
                                    </tbody>
